"""AI 总结模块：通过中转站（OpenAI 兼容接口）调用模型。

分两类操作：
  1. summarize_daily        —— 把某天的帖子原文 -> 当日总结
  2. reduce_period          —— 把若干下级总结（日/月）归并成 上级总结（周/月/年）
  3. summarize_highlights   —— 从一批高互动帖子里提炼"精华"

所有产出都是中文 Markdown，并明确：只整理"作者说了什么"，不替用户做投资决策。
"""
import base64
import json
...
def get_client() -> OpenAI:
    if not config.RELAY_API_KEY:
        raise RuntimeError("未配置 RELAY_API_KEY，请检查 .env")
    return OpenAI(api_key=config.RELAY_API_KEY, base_url=config.RELAY_API_URL)


def call_llm(user_content: str, system_prompt: str = SYSTEM_PROMPT) -> str:
    client = get_client()
    resp = client.chat.completions.create(
        model=config.RELAY_MODEL,
        messages=[
...
def _image_to_data_url(url: str) -> str | None:
    """下载图片转成 base64 data URL。中转站直接拉雪球图床URL经常超时/失败
    （雪球CDN本机直连正常，但中转站服务器出口访问不稳定），改成我们自己下载再内嵌传给视觉模型。"""
    try:
...
def describe_images(image_urls: list[str]) -> str:
    """把一条帖子的配图丢给视觉模型，换回一段客观的中文描述（用于并入总结上下文）。"""
    if not image_urls:
        return ""
...
def ensure_image_desc(post: dict) -> str:
    """取帖子配图描述，命中缓存（posts.image_desc）直接返回，否则调用视觉模型并写回缓存。"""
    ...
    except Exception as e:  # noqa: BLE001 - 视觉调用失败不影响其余总结流程
...
def _images_appendix(posts: list[dict]) -> str:
    """把配图直接拼成 Markdown 图片小节，附在总结末尾。不经过 LLM——LLM 复述/转抄URL
    容易出错甚至编造，这里直接从 posts.images 读原始URL，保证链接100%准确。"""
    sections = []
    for p in posts:
        try:
...
def reduce_period(
    ...
    period_label: "周总结"/"月总结"/"年度归纳"
    parts: [(子周期名, 子总结markdown), ...]
    sub_unit: "日"/"月"，用于提示词措辞
    """
    joined = "\n\n========== {sep} ==========\n\n".format(sep="").join(
