...
- `doc/Phase8候选功能规划.md` —— 部署收尾清单 + 7个候选功能（动机/实现要点/工作量已写清，不必重新调研）
- `doc/Phase9访客账号系统交付报告.md` —— 手机号/微信/邮箱三种访客登录 + 管理后台登录开关；代码已提交未部署验证，含待优化方向（选股并发瓶颈/账号不互通/找回密码缺失等）
- `doc/访客账号体系后续需求PRD.md` —— **下一步做什么先看这个**：忘记密码/账号绑定/短信接入等5项后续需求（动机/方案/工作量已写清）
