...
async def create_group(session: AsyncSession, name: str, user_id: str) -> dict | None:
    """返回新分组；名称已存在则返回 None。"""
    now = int(time.time())
    row = (await session.execute(text(
        """
        INSERT INTO stock_groups (name, created_at, user_id)
        VALUES (:name, :ts, :uid)
        ON CONFLICT (user_id, name) DO NOTHING
        RETURNING id, name, created_at, user_id
        """
    ), {"name": name, "ts": now, "uid": user_id})).mappings().first()
    if not row:
        return None
...
async def _get_or_create_group_id(session: AsyncSession, user_id: str, name: str) -> int:
    row = (await session.execute(text(
        "SELECT id FROM stock_groups WHERE user_id = :uid AND name = :n"
    ), {"uid": user_id, "n": name})).first()
    if row:
        return row[0]
    now = int(time.time())
    row = (await session.execute(text(
        "INSERT INTO stock_groups (name, created_at, user_id) VALUES (:n, :ts, :uid) RETURNING id"
    ), {"n": name, "ts": now, "uid": user_id})).first()
    return row[0]
...
async def sync_auto_groups(session: AsyncSession, user_id: str) -> None:
    """按该用户的 trade_records 自动同步「持仓」和「清仓」分组。

    持仓：当前 hold_qty > 0 的股票。
    清仓：7 天内发生过 hold_qty 清零事件 且 当前 hold_qty == 0 的股票。
    """
    try:
        rows = (await session.execute(text(
            "SELECT code, stock_name, direction, quantity, trade_date"
            " FROM trade_records WHERE user_id = :uid ORDER BY trade_date ASC, id ASC"
        ), {"uid": user_id})).mappings().all()

        hold_qty: dict[str, int] = {}
        stock_name: dict[str, str] = {}
        cleared: dict[str, str] = {}  # code -> cleared_date (YYYY-MM-DD)
        ...
        held = [
            ...
            for c, q in hold_qty.items() if q > 0
        ]
        recently_cleared = [
            {"code": c, "name": stock_name[c]}
        ...
        hold_gid = await _get_or_create_group_id(session, user_id, "持仓")
        await _replace_members(session, hold_gid, held)

        clear_gid = await _get_or_create_group_id(session, user_id, "清仓")
